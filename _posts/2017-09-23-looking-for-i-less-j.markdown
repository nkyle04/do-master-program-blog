...
这里还有个用O(n)时间复杂度来解决第二步的问题：
还是从后往前遍历，定义一个stack，每次从stack中顺序查找满足num[i] > stack.top()的最大值，并把top pop出来。然后判断这个最大值是不是能够使min[i] < max[i] < num[i]，如果满足，则找到了答案。否则把num[i] push到stack中。
这样持续遍历，总共时间复杂度使O(n)，因为对每个元素最多只会进栈一次，出栈一次。
